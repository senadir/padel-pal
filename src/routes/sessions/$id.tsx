...
function RouteComponent() {
  ...
  const { slot, matchId } = Route.useSearch()
  ...
  const showVotingView = session.status === 'voting' || session.status === 'draft'
...
const MatchesView = ({
  ...
  // Wrap match participation to check authentication before allowing join/unjoin
  ...
  return (
    <div className="flex items-center gap-3">
      ...
      <DropdownMenu open={playerDialogOpen} onOpenChange={setPlayerDialogOpen}>
        ...
        {playerDialogOpen && (
          <MatchPlayerOptionDialog player={player} currentMatch={currentMatch} />
        )}
      </DropdownMenu>
    </div>
  )
